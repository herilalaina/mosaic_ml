            ...
            try:
                for name, params in config:
                    if name == "SelectPercentile":
                        pipeline = Pipeline(steps=[(name, feature_selection.SelectPercentile(**params)),
    ...
    def test_SelectKBest(self):
        ...
        def evaluate(config, bestconfig):
            try:
                for name, params in config:
                    if name == "SelectKBest":
                        pipeline = Pipeline(steps=[(name, feature_selection.SelectKBest(**params)),
                                                   ("logistic_regression", linear_model.LogisticRegression())])
                        pipeline = fit_model(pipeline, X_train, y_train)
                        return pipeline.score(X_test, y_test)
                raise Exception("Classifier not found")
    ...
    def test_SelectFpr(self):
        ...
        def evaluate(config, bestconfig):
            try:
                for name, params in config:
                    if name == "SelectFpr":
                        pipeline = Pipeline(steps=[(name, feature_selection.SelectFpr(**params)),
                                                   ("logistic_regression", linear_model.LogisticRegression())])
                        pipeline = fit_model(pipeline, X_train, y_train)
                        return pipeline.score(X_test, y_test)
                raise Exception("Classifier not found")
    ...
    def test_SelectFdr(self):
        ...
        def evaluate(config, bestconfig):
            try:
                for name, params in config:
                    if name == "SelectFdr":
                        pipeline = Pipeline(steps=[(name, feature_selection.SelectFdr(**params)),
                                                   ("logistic_regression", linear_model.LogisticRegression())])
                        pipeline = fit_model(pipeline, X_train, y_train)
                        return pipeline.score(X_test, y_test)
                raise Exception("Classifier not found")
    ...
    def test_SelectFwe(self):
        ...
        def evaluate(config, bestconfig):
            try:
                for name, params in config:
                    if name == "SelectFwe":
                        pipeline = Pipeline(steps=[(name, feature_selection.SelectFwe(**params)),
                                                   ("logistic_regression", linear_model.LogisticRegression())])
                        try:
                            pipeline = fit_model(pipeline, X_train, y_train)
                        except TimedOutExc:
            ...
            try:
                for name, params in config:
                    if name == "SelectFromModel":
                        pipeline = Pipeline(steps=[(name, feature_selection.SelectFromModel(**params)),
                                                   ("logistic_regression", linear_model.LogisticRegression())])
                        pipeline = fit_model(pipeline, X_train, y_train)
                        return pipeline.score(X_test, y_test)
                raise Exception("Classifier not found")
    ...
    def test_RFE(self):
        ...
        def evaluate(config, bestconfig):
            try:
                for name, params in config:
                    if name == "RFE":
                        pipeline = Pipeline(steps=[(name, feature_selection.RFE(**params)),
                                                   ("logistic_regression", linear_model.LogisticRegression())])
                        pipeline = fit_model(pipeline, X_train, y_train)
                        return pipeline.score(X_test, y_test)
                raise Exception("Classifier not found")
